<script lang="ts">
  ...
  <nav class={cn("max-w-[200px] w-full space-y-2 z-50")}>
    <Motion
      whileTap={{
        scale: 0.97,
      }}
    ...
    >
      <button
        ...
        class=" bg-green-400 text-black max-w-[300px] flex gap-2 items-center justify-between p-2.5  outline-none"
      >
        <span class="text-sm">Sort by: design field</span>
        <div style=" transformOrigin: '50% 55%'">
          <Motion animate={svgControls} let:motion>
            <ChevronDown class='stroke-black size-4' />
          </Motion>
        </div>
      </button>
      <Motion
